        ...
        if (!updatedBeer.country || !updatedBeer.birthyear || !updatedBeer.province || !updatedBeer.name) {
            ...
            dbBeer.find({}).toArray(function(err, beers) {
                if (err) {
                    ...
                    var beersBeforeInsertion = beers.filter((beer) => {
                        return (beer.country.localeCompare(country, "en", {'sensitivity': 'base'
                        }) === 0);
                    });
                    if (beersBeforeInsertion.length > 0) {
                        dbBeer.update({
...
// GET a single resource
app.get(BASE_API_PATH + "/beers-stats/:year", function(request, response) {
    var year = parseInt(request.params.year);
    console.log(year);
    if (!year) {
        console.log("WARNING: New GET request to /beers-stats/:year without year, sending 400...");
        response.sendStatus(400); // bad request
    }
    else {
        console.log("INFO: New GET request to /beers-stats/" + year);
        dbBeer.find({
            "birthyear": year
        }).toArray(function(err, filteredBeers) {
            if (err) {
                ...
                if (filteredBeers.length > 0) {
                    ...
                    console.log("WARNING: There are not any year with " + year);
                    response.sendStatus(404); // not found
                }
            }
        });
        ...
        if (!newBeer.country || !newBeer.birthyear || !newBeer.province || !newBeer.name) {
            ...
            dbBeer.find({}).toArray(function(err, beers) {
                if (err) {
                    ...
                    var BeersBeforeInsertion = beers.filter((beer) => {
                        return (beer.year.localeCompare(newBeer.year, "en", {
                            'sensitivity': 'base'
                        }) === 0);
                    });
                    if (BeersBeforeInsertion.length > 0) {
...
//POST over a single resource
app.post(BASE_API_PATH + "/beers-stats/:year", function(request, response) {
    var year = parseInt(request.params.year);
    console.log("WARNING: New POST request to /beers-stats/" + year + ", sending 405...");
    response.sendStatus(405); // method not allowed
});
...
//PUT over a single resource
app.put(BASE_API_PATH + "/beers-stats/:year", function(request, response) {
    var updatedBeer = request.body;
    var year = parseInt(request.params.year);
    console.log();
    if (!updatedBeer) {
        ...
        if (!updatedBeer.country || !updatedBeer.birthyear || !updatedBeer.province || !updatedBeer.name) {
            ...
            dbBeer.find({}).toArray(function(err, beers) {
                if (err) {
                    ...
                    var beersBeforeInsertion = beers.filter((beer) => {
                        return (beer.year.localeCompare(year, "en", {'sensitivity': 'base'
                        }) === 0);
                    });
                    if (beersBeforeInsertion.length > 0) {
                        dbBeer.update({
                        ...
                        console.log("INFO: Modifying beer with year " + year + " with data " + JSON.stringify(updatedBeer, 2, null));
                        response.send(updatedBeer); // return the updated Beer
                    }
                    else {
                        console.log("WARNING: There is not any beer with year " + year);
                        response.sendStatus(404); // not found
                    }
                }
            });
...
//DELETE over a single resource
app.delete(BASE_API_PATH + "/beers-stats/:year", function(request, response) {
    var year= parseInt(request.params.year);
    if (!year) {
        console.log("WARNING: New DELETE request to /beers-stats/:year without year, sending 400...");
        response.sendStatus(400); // bad request
    }
    else {
        console.log("INFO: New DELETE request to /beers-stats/" + year);
        dbBeer.remove({
            "birthyear": year
        }, {}, function(err, result) {
            var numRemoved = JSON.parse(result);
            if (err) {
                ...
                console.log("INFO: Beers removed: " + numRemoved.n);
                if (numRemoved.n === 1) {
                    console.log("INFO: The Beer with year " + year + " has been succesfully deleted, sending 204...");
                    response.sendStatus(204); // no content
                }
                else {
                    console.log("WARNING: There are no beer to delete");
...
// GET a single resource
app.get(BASE_API_PATH + "/establishments/:year", function(request, response) {
