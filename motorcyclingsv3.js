...
module.exports.register_motorcyclings_apiv3 = function(app) {
        ...
        if(CheckKey(keyprovided, response)) {
            ...
            dbMotorcycling.find({}).toArray(function(err, motorcycling) {
                if (err) {
                    ...
                    var motorcyclingToSend = Paginate(motorcycling, limit, offset).filter(Search(yearfrom, yearto));
                    console.log("INFO: Sending motorcycling: " + JSON.stringify(motorcyclingToSend, 2, null));
                    response.send(motorcyclingToSend);
                }
            }); 
        }
    });
    ...
    //Proxy
    app.get(BASE_API_PATH + "/motorcycling-stats/salaryproxy", (req,res)=>{
            ...
            dbMotorcycling.find({
                ...
                if (err) {
                    ...
                    if (filteredMotorcycling.length > 0) {
                        var motor = Paginate(filteredMotorcycling, limit, offset);
            ...
            dbMotorcycling.find({
                ...
                if (err) {
                    ...
                    if (filteredMotorcycling.length > 0) {
                        ...
                        console.log("INFO: Sending motor: " + JSON.stringify(motor, 2, null));
                        response.send(Paginate(motor, limit, offset));
                    }
                    else {
                        console.log("WARNING: There are not any country with " + country);
                        response.sendStatus(404); // not found
        ...
        if (!country || !year) {
            ...
            dbMotorcycling.find({
                ...
                if (err) {
                    ...
                    if (filteredMotorcycling.length > 0) {
                        var motorcycling = Paginate(filteredMotorcycling, limit, offset); //since we expect to have exactly ONE motorcycling with this country or year
